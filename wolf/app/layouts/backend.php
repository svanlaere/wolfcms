...
<html lang="<?php echo(AuthUser::getRecord()->language) ?? 'en'; ?>">
    <head>
        ...
        <title><?php use_helper('Kses');
               echo $title . ' | ' . kses(Setting::get('admin_title'), []); ?></title>

        <link rel="icon" href="<?php echo PATH_PUBLIC; ?>wolf/admin/images/favicon.ico">
        <link rel="preload stylesheet" href="<?php echo PATH_PUBLIC; ?>wolf/admin/stylesheets/admin.css" as="style">
        <link rel="preload stylesheet" href="<?php echo PATH_PUBLIC; ?>wolf/admin/themes/<?php echo Setting::get('theme'); ?>/styles.css" as="style">
        <script rel="preload" src="<?php echo PATH_PUBLIC; ?>wolf/admin/javascripts/cp-datepicker.js"></script>
        <script rel="preload" src="<?php echo PATH_PUBLIC; ?>wolf/admin/javascripts/wolf.js"></script>
        <script rel="preload" src="<?php echo PATH_PUBLIC; ?>wolf/admin/javascripts/jquery-1.8.3.min.js"></script>
        <script rel="preload" src="<?php echo PATH_PUBLIC; ?>wolf/admin/javascripts/jquery-ui-1.10.3.min.js"></script>
        <script rel="preload" src="<?php echo PATH_PUBLIC; ?>wolf/admin/javascripts/jquery.ui.nestedSortable.js"></script>
...
<?php if (file_exists(CORE_ROOT . '/plugins/' . $plugin_id . '/' . $plugin_id . '.js')): ?>
        <script rel="preload" src="<?php echo PATH_PUBLIC; ?>wolf/plugins/<?php echo $plugin_id . '/' . $plugin_id; ?>.js"></script>
<?php endif; ?>
<?php if (file_exists(CORE_ROOT . '/plugins/' . $plugin_id . '/' . $plugin_id . '.css')): ?>
        <link rel="preload stylesheet" href="<?php echo PATH_PUBLIC; ?>wolf/plugins/<?php echo $plugin_id . '/' . $plugin_id; ?>.css" as="style">
<?php endif; ?>
<?php endforeach; ?>
<?php foreach (Plugin::$stylesheets as $plugin_id => $stylesheet): ?>
        <link  rel="preload stylesheet" href="<?php echo PATH_PUBLIC; ?>wolf/plugins/<?php echo $stylesheet; ?>" as="style">
<?php endforeach; ?>
<?php foreach (Plugin::$javascripts as $jscript_plugin_id => $javascript): ?>
        <script rel="preload" src="<?php echo PATH_PUBLIC; ?>wolf/plugins/<?php echo $javascript; ?>"></script>
<?php endforeach; ?>
...
<?php $action = Dispatcher::getAction(); ?>
    ...
    <body id="body_<?php echo $ctrl . '_' . Dispatcher::getAction(); ?>">
        ...
        <div id="header">
            ...
            <div id="mainTabs">
                <ul>
                    <li id="page-plugin" class="plugin"><a href="<?php echo get_url('page'); ?>"<?php if ($ctrl == 'page') {
        echo ' class="current"';
}
?>><?php echo __('Pages'); ?></a></li>
<?php if (AuthUser::hasPermission('snippet_view')): ?>
                    <li id="snippet-plugin" class="plugin"><a href="<?php echo get_url('snippet'); ?>"<?php if ($ctrl == 'snippet') {
        echo ' class="current"';
}
?>><?php echo __('MSG_SNIPPETS'); ?></a></li>
<?php endif; ?>
<?php if (AuthUser::hasPermission('layout_view')): ?>
                    <li id="layout-plugin" class="plugin"><a href="<?php echo get_url('layout'); ?>"<?php if ($ctrl == 'layout') {
        echo ' class="current"';
}
?>><?php echo __('Layouts'); ?></a></li>
<?php endif; ?>
...
<?php Observer::notify('view_backend_list_plugin', $plugin_name, $plugin); ?>
                    <li id="<?php echo $plugin_name; ?>-plugin" class="plugin"><a href="<?php echo get_url('plugin/' . $plugin_name); ?>"<?php if ($ctrl == 'plugin' && $action == $plugin_name) {
        echo ' class="current"';
}
?>><?php echo $plugin->label; ?></a></li>
        <?php endif; ?>
<?php endforeach; ?>

<?php if (AuthUser::hasPermission('admin_edit')): ?>
                    <li class="right"><a href="<?php echo get_url('setting'); ?>"<?php if ($ctrl == 'setting') {
        echo ' class="current"';
}
?>><?php echo __('Administration'); ?></a></li>
<?php endif; ?>
<?php if (AuthUser::hasPermission('user_view')): ?>
                    <li class="right"><a href="<?php echo get_url('user'); ?>"<?php if ($ctrl == 'user') {
        echo ' class="current"';
}
?>><?php echo __('Users'); ?></a></li>
<?php endif; ?>
                </ul>
            </div>
        </div>
<?php if (Flash::get('error') !== null): ?>
                                <div id="error" class="message"><?php echo Flash::get('error'); ?></div>
<?php endif; ?>
<?php if (Flash::get('success') !== null): ?>
                                <div id="success" class="message"><?php echo Flash::get('success'); ?></div>
<?php endif; ?>
<?php if (Flash::get('info') !== null): ?>
                                <div id="info" class="message"><?php echo Flash::get('info'); ?></div>
<?php endif; ?>
        <div id="main">
